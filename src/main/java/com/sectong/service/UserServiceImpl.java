...
public class UserServiceImpl implements UserService
{
		...
		if (file != null)
		{
			if (!file.isEmpty())
			{
				try
				{
					...
					String serverPath = new URL(request.getScheme(), request.getServerName(), request.getServerPort(), request.getContextPath())
							.toString();
	...
	public Object getUserList(int current, int rowCount, String searchPhrase)
	{
		...
		Page<User> users = userRepository.findByUsernameContaining(searchPhrase, new PageRequest(current - 1, rowCount));
		for (User user : users)
		{
